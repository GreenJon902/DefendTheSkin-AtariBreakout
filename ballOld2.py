...
class Ball(Widget):
    ...
    def move(self):
        ...
        #self.racket.center_x = self.center_x
    ...
    def update(self, _=None):
        ...
        if self.top >= self.parent.height * bgSkinBottom:
            x = round_down_to_set_out_puts(self.x, list(self.atariGridXCoords.values()))
            right = round_up_to_set_out_puts(self.right, list(self.atariGridRightCoords.values()))
            top = round_down_to_set_out_puts(self.top, list(self.atariGridYCoords.values()))

            # AtariGrid X
            for brickX in range(atariGridShape[0]):
                if ((x == self.atariGridXCoords[brickX] and self.x <= self.atariGridXCoords[brickX]) or
                    (right == self.atariGridRightCoords[brickX] and self.right >= self.atariGridRightCoords[
                        brickX])) and top == self.atariGridYCoords[self.atariGrid[brickX] - 1]:
                    self.velocity_x *= -1
                ...
                for brickY in range(atariGridShape[1]):
                    ...
                    for brickX in range(atariGridShape[0]):
                        if (atariGridShape[1] - brickY == self.atariGrid[brickX]) and \
                            ...
                            if top == self.atariGridYCoords[brickY]:
                            ...
                            if top == self.atariGridYCoords[brickY]:
                                self.hideBrickFunc(brickX + 1, self.atariGrid[brickX + 1])
